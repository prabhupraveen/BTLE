...
int main(int argc, char *argv[]) {
  ...
  uint32_t total_len = magic_header_len + timestamp_len + access_address_len + length_field_len;
  uint32_t total_len_magic_header = magic_header_len;
  ...
  while ((opt = getopt(argc, argv, "p:")) != -1) {
    switch (opt) {
      case 'p':
        ...
        listen_port = (uint16_t)tmp;
        break;
      default:
        fprintf(stderr, "Usage: ble_fpga_ctl -p <listen_port>\n");
        return EXIT_FAILURE;
    }
  }
