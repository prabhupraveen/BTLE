...
remove_files /home/vishwaksen/pentarisc/projects/clash/BTLE/verilog/sdpram_two_clk_xilinx.v
...
# This command actually loads the core and makes ipx::current_core available
ipx::edit_ip_in_project -upgrade true -name tmp_edit_project -directory $ip_core_dir $ip_core_dir/component.xml
update_compile_order -fileset sources_1

set core [ipx::current_core]

set_property description "BTLE Controller IP Core" $core
set_property core_revision 2 $core

ipx::add_bus_parameter FREQ_HZ [ipx::get_bus_interfaces bb_clk -of_objects $core]
ipx::add_bus_parameter FREQ_HZ [ipx::get_bus_interfaces rf_clk -of_objects $core]
ipx::add_bus_parameter FREQ_HZ [ipx::get_bus_interfaces s00_axi_aclk -of_objects $core]
...
# Add local files from the original project FIRST
set files [list \
...
# THEN generate the block design targets now that system.bd is actually in the project
open_bd_design [get_files [file normalize "${origin_dir}/src/system.bd"]]

# # Fix clk_wiz_1 frequency mismatch
# # set_property -dict [list CONFIG.PRIM_IN_FREQ {100.000}] [get_bd_cells clk_wiz_1]
# set actual_freq [get_property FREQ_HZ [get_bd_pins axi_ad9361/l_clk]]

# if {$actual_freq eq ""} {
#     set freq_mhz 100.000
# } else {
#     set freq_mhz [expr {$actual_freq / 1000000.0}]
# }

# set_property -dict [list CONFIG.PRIM_IN_FREQ $freq_mhz] [get_bd_cells clk_wiz_1]

# Fix the gpio width mismatch
set target_pin [get_bd_pins btle_controller_0/gpio]
set target_net [get_bd_nets -quiet -of_objects $target_pin]

# Disconnect the pin if it is currently connected to anything
if {$target_net ne ""} {
    disconnect_bd_net $target_net $target_pin
}

# Create GND_8 only if it does not already exist
if {[get_bd_cells -quiet GND_8] eq ""} {
    create_bd_cell -type ip -vlnv xilinx.com:ip:xlconstant:1.1 GND_8
    set_property -dict [list CONFIG.CONST_WIDTH {8} CONFIG.CONST_VAL {0}] [get_bd_cells GND_8]
}

# Connect the pin
connect_bd_net [get_bd_pins GND_8/dout] $target_pin
# Suppress the auto-tie-off warnings for the intentionally unconnected pins
set_msg_config -id {BD 41-759} -suppress

validate_bd_design
save_bd_design
generate_target all [get_files [file normalize "${origin_dir}/src/system.bd"]]
