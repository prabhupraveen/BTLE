...
// PHY 2M/test-mode controls:
// - In baremetal mode these come from gpio for quick bring-up.
// - In LL dummy mode these unused signals are mapped to 0 since full LL is not present.
wire phy_2m_mode;
wire [2:0] phy_test_mode;

assign phy_2m_mode = (baremetal_phy_intf_mode ? gpio[1] : 1'b0);
assign phy_test_mode = (baremetal_phy_intf_mode ? gpio[4:2] : 3'b0);
...
(
  ...
  .bram_addr_b_half_flag(bram_addr_b_half_flag),
  .bram_addr_b(bram_addr_b),
  .bram_addr_a({BRAM_ADDR_WIDTH_IN_BYTE{1'b0}}),
  .bram_clk_a(1'b0),
  .bram_wrdata_a({BRAM_DATA_WIDTH{1'b0}}),
  .bram_rddata_a(),
  .bram_en_a(1'b0),
  .bram_rst_a(1'b0),
  .bram_we_a(1'b0)
...
// Tie off upper bits of address wires for tx and rx mem that are wider than dummy's 6 bits
assign ll_tx_pdu_octet_mem_addr[NUM_BIT_PAYLOAD_LENGTH:6] = 0;
assign ll_rx_pdu_octet_mem_addr[NUM_BIT_PAYLOAD_LENGTH:6] = 0;
...
(
  ...
  .tx_pdu_octet_mem_data(ll_tx_pdu_octet_mem_data),
  .tx_pdu_octet_mem_addr(ll_tx_pdu_octet_mem_addr[5:0]),
  .tx_start             (ll_tx_start),
  .tx_iq_valid_last     (tx_iq_valid_last),
  ...
  .rx_decode_end    (ext_rx_decode_end),
  .rx_crc_ok        (ext_rx_crc_ok),
  .rx_payload_length(ext_rx_payload_length[6:0]),

  .rx_pdu_octet_mem_addr(ll_rx_pdu_octet_mem_addr[5:0]),
  .rx_pdu_octet_mem_data(ext_rx_pdu_octet_mem_data)
);
