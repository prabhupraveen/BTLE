...
if __name__ == "__main__":
  ...
  print('arguments: example_idx snr ppm_value num_sample_delay phy_mode(1M|2M)')
  ...
  print([example_idx, snr, ppm_value, num_sample_delay, phy_mode])
  ...
  if example_idx == 0:
    ...
    channel_number = 9
    access_address = '1B0A8560'
    crc_state_init_hex = 'A77B22'
    crc_state_init_bit = bl.hex_string_to_bit(crc_state_init_hex)
    pdu_bit_in_hex = '030c00020f0e50040706d007ffee'
  elif example_idx == 2:
    print('btle_tx command for current example:')
    ...
    print('example_idx must be 0, 1, or 2')
    exit(1)
  ...
  print('phy_mode ', phy_mode)
  print('')

  # start generate test vector for verilog
  bl.SAVE_FOR_VERILOG = 1
  bl.SAVE_DIR = f'../verilog/test_data_{phy_mode}/'
  ...
  # print the bit level processing in transmitter
  ...
  ax.grid(True)
  ax.set_title(f'Signals in BTLE transmitter ({phy_mode})')
  ...
  ax.legend(loc='upper right')
  ax.grid(True)
  ax.set_title(f'BTLE transmitter I and Q ({phy_mode})')
  plt.tight_layout()

  np.savetxt(f'plot_phy_bit_upsample_{phy_mode}.txt', phy_bit_upsample, fmt='%f')
  np.savetxt(f'plot_btle_tx_vco_test_input_{phy_mode}.txt', btle_tx_vco_test_input[8:]/64.0, fmt='%f')
  np.savetxt(f'plot_fo_{phy_mode}.txt', fo[6:], fmt='%f')
  np.savetxt(f'plot_tx_i_{phy_mode}.txt', tx_i[8:], fmt='%f')
  np.savetxt(f'plot_tx_q_{phy_mode}.txt', tx_q[8:], fmt='%f')
  ...
  ax.legend(loc='upper right')
  ax.grid(True)
  ax.set_title(f'signal for decision in receiver ({phy_mode})')
  plt.tight_layout()

  np.savetxt(f'plot_signal_for_decision_x_{phy_mode}.txt', signal_for_decision_idx - idx_shift_left, fmt='%f')
  np.savetxt(f'plot_signal_for_decision_y_{phy_mode}.txt', signal_for_decision, fmt='%f')
  np.savetxt(f'plot_signal_for_decision_best_phase_x_{phy_mode}.txt', signal_for_decision_idx[best_sample_phase_idx::bl.SAMPLE_PER_SYMBOL] - idx_shift_left, fmt='%f')
  np.savetxt(f'plot_signal_for_decision_best_phase_y_{phy_mode}.txt', signal_for_decision[best_sample_phase_idx::bl.SAMPLE_PER_SYMBOL], fmt='%f')
