...
def generate_packet_files(case_name, channel, access_addr, crc_init_hex, pdu_hex, is_random=False):
    """
    Generates a full suite of TX and RX files for a specific packet configuration.
    """
    case_dir = os.path.join(BASE_SAVE_DIR, case_name)
    ...
    print(f"Generating Case: {case_name} | Chan: {channel} | AA: {access_addr} | CRCInit: {crc_init_hex}")
    ...
    if not crc_ok and not is_random:
        print(f"  [!] Warning: CRC failed for golden case {case_name}")
...
if __name__ == "__main__":
    ...
    for case in golden_cases:
        generate_packet_files(case["name"], case["chan"], case["aa"], case["crc"], case["pdu"])
